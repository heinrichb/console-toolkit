...
export function createProgressBar(options: ProgressBarOptions): PrintLine {
	...
	const resolvedFillStyle = p >= 1 ? (completeStyle ?? fillStyle ?? resolvedBarStyle) : (fillStyle ?? resolvedBarStyle);
	...
	const effectiveFillChar = p >= 1 ? (completeChar ?? fillChar) : fillChar;
