...
- **Explain the 'Why':** When documenting code, focus on the _reasoning_ behind decisions, trade-offs, and future goals, not just what the code does.
...
- **Demo Script:** The `src/demo.ts` script must demonstrate _all_ capabilities of the library and be kept up-to-date with every feature addition or change.
...
Before submitting _any_ changes:
...
- **Prompt-Ready:** Write descriptions that can be copy-pasted as the _first prompt_ for a new agent session.
- **Format:**
